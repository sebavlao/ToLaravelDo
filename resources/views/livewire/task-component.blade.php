<section class="grid gap-6 max-w-4xl">
    ...
    @foreach ($tasks as $task)
        @php $color = $this->colorTaskHandle($task->status) @endphp
        <article class="flex justify center p-4 items-center rounded-xl" style="background-color: var(--skin-{{$color}})">
            <span>{{ $iconOptions[$task->icon] }}</span>
            <div>{{ $task->title }}</div>
            <div class="p-2 rounded-xl" style="background-color: var(--skin-{{$color}}-contrast)">
                ...
                @elseif ($task->status === "In_Progress")
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <circle cx="10" cy="10" r="7.5" fill="#000000" fill-opacity="0.25"/>
                ...
                @elseif ($task->status === "Wont_do")
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <circle cx="10" cy="10" r="7.5" fill="#000000" fill-opacity="0.25"/>
                        <path d="M7.5 7.5L12.5 12.5" fill="#000000" stroke-width="1.2" stroke-linecap="round"/>
                        <path d="M12.5 7.5L7.5 12.5" fill="#000000" stroke-width="1.2" stroke-linecap="round"/>
                    </svg>
                @endif
            </div>
        </article>
